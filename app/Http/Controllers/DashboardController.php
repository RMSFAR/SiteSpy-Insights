...
class DashboardController extends HomeController
{
    ...
    */
    ...
    {  
        ...
        $query = DB::table('visitor_analysis_domain_list');
        $visitor_domain_session_value = session('visitor_doamin_name_id');
        if(isset($visitor_domain_session_value)){
            $query = $query->where('id',$visitor_domain_session_value);
        }
        
        $visitor_type_data= $query->where("user_id", Auth::user()->id)->where("dashboard", '1')->select("id","domain_name")->first();
        if(isset($visitor_type_data)){
            session(['visitor_doamin_name_id'=>$visitor_type_data->id]);
            session(['session_visitor_doamin_name'=>$visitor_type_data->domain_name]);
        }
        $line_char_data = [];
        $i=1;
            ...
            $query2 = DB::table('visitor_analysis_domain_list_data')
                    ->where("date_time", ">=", $from_date_7)
                    ->where("date_time", "<=", $to_date);
            if(isset($visitor_type_data)) $query2 = $query2->where("domain_list_id", $visitor_type_data->id);
            $domain_all_data = $query2->select($select)->get();
            $domain_all_data = json_decode(json_encode($domain_all_data));
            ...
            usort($country_report_data, function ($a, $b) {
                if ($a['user_number'] == $b['user_number']) {
                    return 0;
                }
                return ($a['user_number'] < $b['user_number']) ? -1 : 1;
            ...
            $data[$data_number] = $temp_data;
            $data[$website_name] = $visitor_type_data->domain_name ?? '';
            $data[$website_id] = $visitor_type_data->id ?? '';
            // end of PieChart
            ...
            for($j=0;$j<=$no_of_days;$j++){
                ...
                $line_char_data[$i-1][$j]['domain'] = $visitor_type_data->domain_name ?? '';
            }
            // end of lineChart data
            $i++;
        ...
        $query3 = DB::table('visitor_analysis_domain_list_data'); 
        if(isset($visitor_type_data)) $query3 = $query3->where("domain_list_id", $visitor_type_data->id);
        $all_data =$query3->select('id', 'last_scroll_time', 'last_engagement_time', 'cookie_value', 'session_value', 'date_time', 'referrer', 'visit_url', 'browser_name', 'device', 'country', 'os', 'is_new')
        ...
        if(!empty($stay_time_info)) {
            foreach($stay_time_info as $value){
                ...
                if(empty($value['last_scroll_time']) && empty($value['last_engagement_time']))
                    $total_stay_time = $total_stay_time + $total_stay_time_individual;
                else if (empty($value['last_scroll_time']) && !empty($value['last_engagement_time'])){
                    $total_stay_time_individual = strtotime($value['last_engagement_time']) - strtotime($value['stay_from']);
                    $total_stay_time = $total_stay_time + $total_stay_time_individual;
                }
                else if (!($value['last_scroll_time']) && empty($value['last_engagement_time'])){
        ...
        usort($top_bowser_array, function ($a, $b) {
        ...
        foreach($country_name as $value){
            ...
            $searchValue = strtolower(trim($value["country"]));
            $lowercaseCountryList = array_map('strtolower', $country_list);
            $country_flag = in_array($searchValue, $lowercaseCountryList) ? array_search($searchValue, $lowercaseCountryList) : $searchValue;
            ...
            $image_link = asset("assets/img/flags/".$country_flag.".png");
        ...
        usort($top5_country, function ($a, $b) {
    ...
    public function visior_domain_session(Request $request)
    {
        $id = $request->id;
        $name = $request->name;
        session(['visitor_doamin_name_id'=>$id]);
        session(['session_visitor_doamin_name'=>$name]);
        return true;
    }
