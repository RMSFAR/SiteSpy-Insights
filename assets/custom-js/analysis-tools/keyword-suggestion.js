...
$(document).ready(function($) {
  ...
  $(document).on('click','#download_btn',function(event){
    event.preventDefault();
      ...
      $.ajax({
        type:'POST',
        url:keyword_suggestion_download,
        beforeSend: function (xhr) {
          xhr.setRequestHeader('X-CSRF-TOKEN', csrf_token);
        },
        data:{ids:ids},
      ...
      .then((willDelete) => {
        if (willDelete) 
        {
          ...
          $.ajax({
            context: this,
            type:'POST' ,
            url:keyword_suggestion_delete,
            beforeSend: function (xhr) {
              xhr.setRequestHeader('X-CSRF-TOKEN', csrf_token);
            },
            data:{ids:ids},
            success:function(response){ 
              iziToast.success({title: '',message: Your_keyword_auto_suggestion_data_has_been_deleted_successfully,position: 'bottomRight'});
              table.draw();
            }
          });
        } 
      ...
      .then((willDelete) => {
        if (willDelete) 
        {
          ...
          $.ajax({
            ...
            data:{ids:ids},
            success:function(response){ 
              iziToast.success({title: '',message: Your_all_keyword_auto_suggestion_data_has_been_deleted_successfully,position: 'bottomRight'});
              table.draw();
            }
          });
        } 
