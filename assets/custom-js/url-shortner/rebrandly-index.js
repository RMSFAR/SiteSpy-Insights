...
$(document).ready(function($) {
	...
	$(document).on('click','#download_btn',function(event){
		event.preventDefault();
			...
			if(ids.length==0) 
			{
			swal(global_lang_error, You_have_to_select_list_from_data_table , 'warning');
			return false;
			}
			...
			$.ajax({
				type:'POST',
				url:rebrandly_short_url_download,
				beforeSend: function (xhr) {
                    xhr.setRequestHeader('X-CSRF-TOKEN', csrf_token);
                },
				data:{ids:ids},
	...
	$(document).on('click','#delete_btn',function(e){
		...
		if (willDelete) 
		{
			...
			context: this,
			type:'POST' ,
			url:rebrandly_short_url_delete,
			beforeSend: function (xhr) {
				xhr.setRequestHeader('X-CSRF-TOKEN', csrf_token);
			},
			data:{ids:ids},
			success:function(response){ 
				iziToast.success({title: '',message: Your_rebrandly_shortener_data_has_been_deleted_successfully , position: 'bottomRight'});
				table.draw();
			}
			});
		} 
	...
	$(document).on('click','#delete_btn_all',function(e){
		...
		if (willDelete) 
		{
			...
			data:{ids:ids},
			success:function(response){ 
				iziToast.success({title: '',message: Your_all_rebrandly_shortener_data_has_been_deleted_successfully,position: 'bottomRight'});
				table.draw();
			}
			});
		} 
