{{-- @extends('layouts.app') --}}
@extends('design.app')
@section('title',__('Rebrandly URL shortener'))
@section('content')

<link rel="stylesheet" href="{{asset('assets/custom-css/url-shortner/rebrandly-index.css')}}">

<section class="section section_custom">
	<div class="section-header">
		<h1><i class="fa fa-cut"></i> {{ __('Rebrandly URL shortener')}}</h1>
		<div class="section-header-button">
			<a class="btn btn-primary" href="{{route("rebrandly")}}">
		...
		<div class="section-header-breadcrumb">
			...
			<div class="breadcrumb-item">{{ __('Rebrandly URL shortener')}}</div>
		</div>
	</div>
...
<script>
	"use strict"  
	var rebrandly_shortener_data = '{{ route('rebrandly_shortener_data') }}';
	var rebrandly_short_url_download = '{{ route('rebrandly_short_url_download') }}';
	var rebrandly_short_url_delete = '{{ route('rebrandly_short_url_delete') }}';
	var Your_rebrandly_shortener_data_has_been_deleted_successfully = '{{ __("Your rebrandly shortener data has been deleted successfully.") }}';
	var Your_all_rebrandly_shortener_data_has_been_deleted_successfully = '{{ __("Your All rebrandly url shortener data has been deleted successfully.") }}';

</script>

<script src="{{asset('assets/custom-js/url-shortner/rebrandly-index.js')}}"></script>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-cut"></i> {{ __('Rebrandly URL Shortener')}}</h5>
