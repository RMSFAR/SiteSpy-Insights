...
<section class="section">
  <div class="section-header">
    <h1><i class="fa fa-server"></i> {{ __('Server Information')}}</h1>
    <div class="section-header-breadcrumb">
      <div class="breadcrumb-item"><a href="{{route('analysis_tools') }}">{{ __("Analysis Tools")}}</a></div>
      <div class="breadcrumb-item">{{ __('Server Information')}}</div>
    </div>
  </div>
</section>
      ...
      <form  method="POST" enctype="multipart/form-data"  id="new_search_form">
        ...
        <div class="card-body">
          ...
          <div class="form-group">
              <label class="form-label"> {{ __("Domain")}} <code>*</code> <a href="#" data-placement="top" data-toggle="popover" data-trigger="focus" title="" data-content="Put your domain names / urls comma separated" data-original-title="Domain"><i class="fa fa-info-circle"></i> </a></label>
...
<script>    
  ...
  var server_info_action = '{{ route('server_info_action') }}';

</script>

<script src="{{asset('assets/custom-js/analysis-tools/server-info.js')}}"></script>
