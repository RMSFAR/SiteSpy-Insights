...
<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
     <a class="btn btn-primary" href="{{route("page_status_new")}}">
          ...
          <div class="card-body data-card">
            ...
            <div class="table-responsive2">
                ...
                <thead>
                    ...
                    <th>{{ __("Status")}}</th>
                    <th>{{ __("Total Time")}}</th>
                    <th>{{ __("Name Lookup Time")}}</th>
                    <th>{{ __("Connect Time")}}</th>
                    <th>{{ __("Download Speed")}}</th>
                    <th>{{ __("Check At")}}</th>
...
<script>
  "use strict";
  var Your_page_status_check_data_has_been_deleted_successfully = '{{ __('Your page status check data has been deleted successfully.') }}';
  var Your_all_page_status_check_data_has_been_deleted_successfully = '{{ __('Your all page status check data has been deleted successfully.') }}';
  var page_status_list_data = '{{ route('page_status_list_data') }}';
  var page_status_download = '{{ route('page_status_download') }}';
  var page_status_delete = '{{ route('page_status_delete') }}';
</script>


<script src="{{asset('assets/custom-js/analysis-tools/pagestattus.js')}}"></script>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-anchor"></i> {{ __('Page Status')}}</h5>
