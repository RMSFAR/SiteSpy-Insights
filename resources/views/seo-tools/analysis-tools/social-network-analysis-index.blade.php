...
<section class="section section_custom">
  <div class="section-header">
    ...
    <a class="btn btn-primary" href="{{route('social_network_analysis')}}">
              ...
              <table class="table table-bordered" id="mytable">
                <thead>
                  <tr>
                    ...
                    <th>{{ __("ID")}}</th>            
                    <th>{{ __("Domain")}}</th>         
                    <th>{{ __("Facebook Share")}}</th>
                    <th>{{ __("Facebook Reaction")}}</th>
                    <th>{{ __("Facebook Comment")}}</th>
                    <th>{{ __("Xing Share")}}</th>
                    <th>{{ __("Pinterest Pin")}}</th>
                    <th>{{ __("Reddit Score")}}</th>
                    <th>{{ __("Reddit UP")}}</th>
                    <th>{{ __("Reddit Down")}}</th>
                    <th>{{ __("Buffer Share")}}</th>
...
<script>
  "use strict";
  var social_list_data = '{{ route('social_list_data') }}';
  var social_delete = '{{ route('social_delete') }}';
  var social_download = '{{ route('social_download') }}';
  var Your_social_network_analysis_data_has_been_deleted_successfully = '{{ __('Your social network analysis data has been deleted successfully.') }}';
  var Your_all_social_network_analysis_data_has_been_deleted_successfully = '{{ __('Your all social network analysis data has been deleted successfully.') }}';

</script>

<script src="{{asset('assets/custom-js/analysis-tools/social-index.js')}}"></script>

<div class="modal fade show" id="social_network_download_selected">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background: #fefefe;">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-share-alt"></i> {{ __('Social Network Analysis')}}</h5>
