	...
	<div class="section-body">
		<div class="card">
          <div class="card-header">
          	<h4 style="width: 100%"><i class="fas fa-toolbox"></i>&nbsp;<?php echo config('my_config.product_short_name').' '.__("Updates");?> <code class="float-right"><?php echo __('Your Version');?> : <b>v<?php echo $current_version; ?></b></code></h4>
          </div>
        		...
        		{ ?>       
		        	<div class="table-responsive2">
		        		<table class='table table-bordered table-striped table-md'>
			        		<tr class='head'>
			        			<th style='text-align: center;'><?php echo __('Version');?></th>
			        			<th style='text-align: center;'><?php echo __('Change Log');?></th>
 		...
 		foreach($add_ons as $add_on)
 		{
 			...
 			<div class="card d-none">
	          <div class="card-header">
	          	<h4 style="width: 100%"><i class="fas fa-plug"></i>&nbsp;<?php echo $add_on->add_on_name.' '.__("Updates");?> <code class="float-right"><?php echo __('Your Version');?> : <b>v<?php echo $add_on->version; ?></b></code></h4>
	          </div>
	          <div class="card-body">
        		<?php
				        		...
				        		foreach($this_update_version as $add_on_update_version)
				        		{
				        			...
				        			<tr>
				        				...
				        				<td style='text-align: center;'>
				        					...
				        					<div id="<?php echo $modal; ?>" class="modal fade"  tabindex="-1" role="dialog"  tabindex="-1" role="dialog" >
				        					  <div class="modal-dialog modal-lg">
				        					    ...
				        					    <div class="modal-content">
				        					      <div class="modal-header">				        					        
				        					        <h5 class="modal-title"><?php echo $add_on_update_version->name; ?> <?php echo $add_on_update_version->version; ?> ( <?php echo __('Change Log');?> )</h5>
				        					        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
				        					          <span aria-hidden="true">&times;</span>
				        					        </button>
				        					      </div>
				        				...
				        				<td style='text-align: center;'>
				        					...
				        					if($k == 1)
				        					{?>
			        							<button id="<?php echo 'addonupdate' . $add_on['id']; ?>" class='btn btn-outline-primary' folder="<?php echo $add_on['unique_name']; ?>" updateid="<?php echo $add_on_update_version->id; ?>" version="<?php echo $add_on_update_version->version; ?>"><i class="fas fa-leaf"></i> <?php echo __('Update Now');?></button>
				        						<?php
				        					}
				        					else
				        					{ ?>
				        						<button disabled='disabled' class='btn btn-outline-primary' updateid="<?php echo $add_on_update_version->id; ?>" version="<?php echo $add_on_update_version->version; ?>"><i class="fas fa-leaf"></i> <?php echo __('Update Now');?></button>
				        						<?php
				        					} ?>
				        				</td>
				        			</tr>
...
<script>
    ...
    var Update_System = '{{ __("Update System") }}';
    var You_are_about_to_update_system_files_and_database = '{{ __("You are about to update system files and database.") }}';
    var Update_Add_on = '{{ __("Update Add-on") }}';
    var You_are_about_to_update_add_on_file_and_database = '{{ __("You are about to update add-on files and database.") }}';
    var do_not_close_this_window_or_refresh_page_untill_update_done = '{{ __("do not close this window or refresh page untill update done.") }}';
...
<script>
	$(document).ready(function()
	{
		$('.update').click(function()
		{
			swal({
			      title: Update_System,
			      text: You_are_about_to_update_system_files_and_database,
			      icon: 'warning',
			      buttons: true,
			      dangerMode: true,
			    })
			    .then((willDelete) => {
			      if (willDelete) {
	      			...
	      			{				
	      				...
	      				var warning_msg=do_not_close_this_window_or_refresh_page_untill_update_done;
	      				var loading = warning_msg+'<br/><br/><img src="'+preloader_img+'" class="center-block" height="30" width="30">';
	      				...
	      				$.ajax({
	                          type: "POST",
	      					data: data,
	      					url: base_url+"/update_system/initialize_update",
	      					dataType: 'JSON',
		...
		$("<?php echo '#addonupdate' . $add_on->id; ?>").click(function()
		{
			swal({
			      title: Update_Add_on,
			      text: You_are_about_to_update_add_on_file_and_database,
			      icon: 'warning',
			      buttons: true,
			      dangerMode: true,
			    })
			    .then((willDelete) => {
			      if (willDelete) {
      				...
      				{				
      					$("#update_success").modal();
      					var warning_msg=do not close this window or refresh page untill update done;
